...
Key 和设置保存在 `chrome.storage.sync`，卸载重装、更新版本都不会丢失；如果浏览器已登录账号，会随浏览器的扩展数据一起在你的设备间同步，未登录则只保留在本机。翻译 Key 只会发送给对应的翻译服务；当弹窗检测到无字幕的 YouTube 视频时，ElevenLabs Key 会被用于换取一个短时效的转写 token；音频只有在你明确点击开始后才会被采集和发送。
